...
func ServiceRuntimePath(method string, requestPath string) bool {
	method = strings.ToUpper(strings.TrimSpace(method))
	path := strings.TrimRight(strings.TrimSpace(requestPath), "/")
	switch path {
	case "/api/leases/acquire", "/api/leases/release":
		return method == http.MethodPost
	case "/api/proxy_exit_ip", "/api/proxy_exit_geo", "/api/ip_fraud_check", "/api/check_cf_access_risk", "/api/target_connectivity_check":
		return method == http.MethodPost
