...
func (r *Runtime) snapshot(ctx context.Context) (*proxyruntimev1.ProxyPoolSnapshot, error) {
	nodes, refreshedAt := r.poolSnapshot.current()
...
func (r *Runtime) gateway(ctx context.Context) (*proxyruntimev1.EgressGateway, error) {
	...
	dataPlaneRoute, err := r.dataPlaneRoute(pool)
	if err != nil {
		return nil, err
	}
	...
	return &proxyruntimev1.EgressGateway{GatewayId: "default", Listeners: r.protoListeners(pool.GetDynamicLeases()), Pool: pool, DataPlaneRoute: dataPlaneRoute, ControlPlaneRoute: controlRoute, ProviderControlPlane: &proxyruntimev1.ProviderControlPlaneAccess{UsesProxy: r.cfg.ProviderHTTPProxy != "", ProxyRef: providerHTTPProxyRef(r.cfg.ProviderHTTPProxy), Protocols: []proxyruntimev1.ProxyProtocol{proxyruntimev1.ProxyProtocol_PROXY_PROTOCOL_HTTP}}, UpdatedAt: timestamppb.Now(), Overview: &proxyruntimev1.ProxyRuntimeOverview{RouteRuntime: routeRuntimeKind(r.dataPlane.Name()), RouteRuntimeStatus: r.dataPlaneStatus(), ProviderAccountCount: uint32(len(accounts)), SourceCount: uint32(len(pool.GetSources())), ActiveLeaseCount: active, UpdatedAt: timestamppb.Now(), SourceRuntime: proxyruntimev1.ProxySourceRuntimeKind_PROXY_SOURCE_RUNTIME_KIND_NONE, SourceRuntimeStatus: "merged into data plane"}}, nil
}

func (r *Runtime) dataPlaneRoute(pool *proxyruntimev1.ProxyPoolSnapshot) (*proxyruntimev1.EgressRoute, error) {
