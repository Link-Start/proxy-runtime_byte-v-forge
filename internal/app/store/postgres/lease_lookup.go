...
func (s *Store) LeaseFactByID(ctx context.Context, leaseID string) (*proxygatewayv1.ProxyDynamicLease, error) {
	row := s.pool.QueryRow(ctx, `SELECT lease_json::text FROM proxy_gateway_dynamic_leases WHERE lease_id=$1`, strings.TrimSpace(leaseID))
...
LIMIT 1
`, strings.TrimSpace(accountID), proxygatewayv1.ProxyDynamicLeaseStatus_PROXY_DYNAMIC_LEASE_STATUS_ACTIVE.String())
	return scanLeaseFact(row)
}
...
func (s *Store) ActiveLeaseFactBySession(ctx context.Context, accountID string, purpose string, sessionID string) (*proxygatewayv1.ProxyDynamicLease, error) {
	...
	args := []any{strings.TrimSpace(accountID), proxygatewayv1.ProxyDynamicLeaseStatus_PROXY_DYNAMIC_LEASE_STATUS_ACTIVE.String(), sessionID}
...
func (s *Store) leaseFactByAccount(ctx context.Context, accountID string, purpose string, activeOnly bool) (*proxygatewayv1.ProxyDynamicLease, error) {
	...
	if activeOnly {
		args = append(args, proxygatewayv1.ProxyDynamicLeaseStatus_PROXY_DYNAMIC_LEASE_STATUS_ACTIVE.String())
