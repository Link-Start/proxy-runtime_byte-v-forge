import type {
  ProxyGatewayMetricsSummary,
  ProxyGatewayOperationMetric,
} from '~/types/byte/v/forge/contracts/proxygateway/v1/proxy_gateway'

export interface ProxyGatewayMetricRow {
  operation: string
  status: string
  count: number
  slowCount: number
...
export interface ProxyGatewayMetricOverview {
  totalCount: number
  errorCount: number
  slowCount: number
  durationSeconds: number
...
export function proxyGatewayMetricRows(
  summary: ProxyGatewayMetricsSummary | undefined,
): ProxyGatewayMetricRow[] {
  return (summary?.operations || []).map(metricRow).sort(compareMetricRows)
}

export function proxyGatewayMetricsOverview(
  rows: ProxyGatewayMetricRow[],
): ProxyGatewayMetricOverview {
...
export function proxyGatewayMetricOperationLabel(operation: string) {
  return operationLabels[operation] || operation.replaceAll('_', ' ')
}

export function proxyGatewayMetricStatusLabel(status: string) {
  switch (status) {
    case 'success':
      return '成功'
    case 'canceled':
...
export function proxyGatewayMetricStatusClass(status: string) {
  switch (status) {
    case 'success':
...
export function formatProxyGatewayMetricCount(value: number) {
  return new Intl.NumberFormat().format(Math.round(value))
}

export function formatProxyGatewayMetricPercent(value: number) {
  if (!Number.isFinite(value)) return '0%'
  return `${(value * 100).toFixed(value > 0 && value < 0.01 ? 2 : 1)}%`
}

export function formatProxyGatewayMetricSeconds(value: number) {
  if (!Number.isFinite(value) || value <= 0) return '0ms'
  if (value < 1) return `${Math.round(value * 1000)}ms`
  return `${value.toFixed(value < 10 ? 2 : 1)}s`
}

export function formatProxyGatewayMetricTime(value: string | undefined) {
  if (!value) return '-'
...
function metricRow(item: ProxyGatewayOperationMetric): ProxyGatewayMetricRow {
...
function compareMetricRows(a: ProxyGatewayMetricRow, b: ProxyGatewayMetricRow) {
