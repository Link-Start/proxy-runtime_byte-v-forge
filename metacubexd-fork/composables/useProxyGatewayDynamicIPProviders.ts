...
} from '~/composables/proxyGatewayDynamicIPHelpers'

export function useProxyGatewayDynamicIPProviders() {
  const api = useProxyGatewayApi()
  const state = useProxyGatewayDynamicIPState()
  ...
  async function load() {
    ...
    try {
      ...
      const [providerRes, settingsRes, accountRes] = await Promise.all([
        api.listProviders(requestOptions),
        api.getSettings(requestOptions),
        api.listProviderAccounts(requestOptions),
      ])
...
export type ProxyGatewayDynamicIPProvidersState = ReturnType<
  typeof useProxyGatewayDynamicIPProviders
>
