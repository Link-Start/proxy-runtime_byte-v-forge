...
import { type MihomoEgressOwner, mihomoOwnersFromController } from '~/composables/proxyGatewayMihomoOwnerHelpers'
import {
  exitText,
  lineText,
...
import {
  ...
  newInUserRuleForm,
} from '~/composables/proxyGatewayInUserRuleForm'
import { isProxyGatewayPlaygroundRule } from '~/composables/proxyGatewayPlaygroundRule'

export function useProxyGatewayInUserRules() {
  ...
  const rows = computed(() =>
    allRows.value.filter(({ rule }) => !isProxyGatewayPlaygroundRule(rule)),
  )
  ...
  async function deleteRule(rule: ProxyIngressRuleSettings) {
    if (isProxyGatewayPlaygroundRule(rule)) return
    await withSave(async () => {
...
export type ProxyGatewayInUserRulesState = ReturnType<typeof useProxyGatewayInUserRules>
