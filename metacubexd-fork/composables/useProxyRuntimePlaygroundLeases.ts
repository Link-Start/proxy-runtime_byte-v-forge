...
const playgroundPurpose = 'in-user-profile'
...
export function useProxyRuntimePlaygroundLeases(
  ...
  const rows = computed(() =>
    leases.value
      .filter(
        (lease) =>
          lease.account_id === profileID.value &&
          lease.status ===
  ...
  const canAcquire = computed(() => playgroundDynamicIPConfigured(runtime))
  const acquireDisabledReason = computed(() => playgroundAcquireDisabledReason(runtime))
  const activeRows = computed(() =>
    rows.value.filter(
      (lease) =>
        lease.status === ProxyDynamicLeaseStatus.PROXY_DYNAMIC_LEASE_STATUS_ACTIVE,
    ),
  )
  ...
  async function acquire() {
    ...
    await withBusy(async () => {
      await persist()
      const policy = playgroundLeasePolicy(runtime.form, profileID.value)
      try {
...
function playgroundDynamicIPConfigured(runtime: ProxyRuntimeInUserRulesState) {
  return playgroundAcquireDisabledReason(runtime) === ''
}
...
function playgroundAcquireDisabledReason(runtime: ProxyRuntimeInUserRulesState) {
  if (
    ...
    return 'PlayGround 未配置动态 IP 出口'
  }
  if (runtime.form.exit_dynamic_session_mode !== ProxySessionMode.PROXY_SESSION_MODE_STICKY) {
    return 'PlayGround 租约只支持粘性动态 IP'
  }
  const providers = runtime.dynamicProviderOptions.value.filter(
    (provider) => (provider.endpoints || []).length > 0,
  )
  if (providers.length === 0) return '没有可用的动态 IP Provider'
  const providerID = runtime.form.exit_dynamic_provider_id.trim()
  const selectedProvider = providerID
    ? providers.find((provider) => provider.dynamic_provider_id === providerID)
    : undefined
  ...
  const endpointExists = endpointProviders.some((provider) =>
    (provider.endpoints || []).some(
      (endpoint) => endpointIDFromURL(endpoint.endpoint_url || '') === endpointID,
    ),
  )
  return endpointExists ? '' : '当前动态 IP 端点不可用'
}

function playgroundLeasePolicy(
  form: ProxyRuntimeInUserRulesState['form'],
  profileID: string,
): ProxySessionPolicy {
  const policy = dynamicIPPolicy(form) as ProxySessionPolicy
  policy.labels = {
    ...(policy.labels || {}),
    selection_seed: profileID,
  }
...
export type ProxyRuntimePlaygroundLeasesState = ReturnType<
  typeof useProxyRuntimePlaygroundLeases
>
